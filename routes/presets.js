...
// Equipment presets with default tasks
// trigger_type: 'time' uses frequency_value + frequency_unit
// trigger_type: 'usage' uses usage_interval + usage_unit
const PRESETS = {
  'HVAC / Air Handler': {
    icon: '❄️',
    usage_unit: null,
  ...
  'Washing Machine': {
    ...
    usage_unit: null,
    tasks: [
      { name: 'Clean drum (front-load)',     description: 'Run clean cycle with washer cleaner tablet. Wipe door seal after every use to prevent mold.', trigger_type: 'time', frequency_value: 1,  frequency_unit: 'month' },
  ...
  'Vehicle / Car': {
    ...
    tasks: [
      ...
      { name: 'Tire rotation',               description: 'Rotate tires in recommended pattern (check manual). Inspect tread depth and pressure.', trigger_type: 'usage', usage_interval: 7500,  usage_unit: 'miles' },
      ...
      { name: 'Cabin air filter',            description: 'Replace cabin air filter — usually behind glove box. Check owner manual for location.', trigger_type: 'usage', usage_interval: 15000, usage_unit: 'miles' },
      { name: 'Brake inspection',            description: 'Inspect brake pad thickness and rotor condition. Replace pads at 2-3mm or per feel/sound.', trigger_type: 'usage', usage_interval: 25000, usage_unit: 'miles' },
  ...
  'Lawnmower': {
    ...
    usage_unit: 'hours',
    tasks: [
      { name: 'Change oil',                  description: 'Drain and replace with SAE 30 (or per manual). First change at 5 hrs on new engine, then every 50 hrs.', trigger_type: 'usage', usage_interval: 50,   usage_unit: 'hours' },
      ...
      { name: 'Sharpen / replace blade',     description: 'Disconnect spark plug wire before working under deck. Sharpen until no nicks remain.', trigger_type: 'time',  frequency_value: 1,   frequency_unit: 'year'  },
      { name: 'Replace spark plug',          description: 'Check gap with feeler gauge (per manual). Replace if electrode worn or deposits heavy.', trigger_type: 'time',  frequency_value: 1,   frequency_unit: 'year'  },
      { name: 'Drain fuel for storage',      description: 'At end of season, run engine dry or add fuel stabilizer. Clean deck of clippings.', trigger_type: 'time',  frequency_value: 1,   frequency_unit: 'year'  },
    ]
  },
  'Generator': {
    icon: '⚡',
  ...
  'Pool / Spa Pump': {
    ...
    tasks: [
      ...
      { name: 'Backwash / clean filter',     description: 'Sand filter: backwash when PSI rises 8-10 above clean baseline. Cartridge: rinse monthly.', trigger_type: 'time', frequency_value: 1,  frequency_unit: 'month' },
      { name: 'Inspect pump seals / bearings',description:'Listen for grinding or squealing. Check for water leaks around seal plate.', trigger_type: 'time', frequency_value: 1,  frequency_unit: 'year'  },
    ]
  },
  'Sump Pump': {
    icon: '💧',
  ...
  'Garage Door Opener': {
    ...
    tasks: [
      ...
      { name: 'Replace batteries',           description: 'Replace remote and wall keypad batteries. Consider backup battery for opener itself.', trigger_type: 'time', frequency_value: 2,  frequency_unit: 'year'  },
    ]
  },
};

// GET /api/presets — return all preset types (names + icons for dropdown)
router.get('/', (req, res) => {
...
// GET /api/presets/:name — return full preset including tasks
